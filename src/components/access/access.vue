<!--
-- Created by crish on 2020/01/28
-->
      ...
      Col(span="8")
        Card
          ...
          div(class="access-user-con access-current-user-con")
            Avatar(style="color: #f56a00;background-color: #fde3cf;" size="large") {{ userName }}
            p 当前用户权限值:
              b {{ accessCode }}
      Col(span="16" class="padding-left-10")
        Card
          ...
          div(class="access-user-con access-change-access-con")
            ...
            Col(span="16" class="padding-left-10")
              Row(type="flex" justify="center" align="middle" class="access-change-access-con-row")
                p 您可以通过左侧的开关来切换当前用户的权限值，然后您可以观察左侧菜单栏的变化，如果当前用户的权限值是
                  b 0
                  span ，则左侧菜单栏会显示’权限测试页‘这一项('权限测试页'只用于测试，点击不会跳转)。
</template>
