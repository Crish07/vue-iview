              ...
              Card(dis-hover)
                ...
                div(style="height: 360px;")
                  ul(id="todoList" class="iview-admin-draggable-list")
                    li(v-for="(item, index) in todoArray" :key="index" class="notwrap todolist-item" :data-index="index") {{ item.content }}
        ...
        Card
          ...
          div(style="height: 394px;")
            ul(class="iview-admin-draggable-list")
              li(v-for="(item, index) in doArray" :key="index" class="notwrap" :data-index="index") {{ item.content }}
              ...
              Card(dis-hover)
                div(style="height: 360px;")
                  ul(id="shoppingList" class="iview-admin-draggable-list")
                    li(v-for="(item, index) in shoppingList" :key="index" class="notwrap todolist-item" :data-index="index") 好吃的美食--{{ item.name }}
      Col(span="8" class="padding-left-10")
        Card
          p(slot="title")
            Icon(type="android-funnel")
            span 买得起的清单
          div(style="height: 394px;")
            ul(class="iview-admin-draggable-list")
              li(v-for="(item, index) in affordList" :key="index" class="notwrap" :data-index="index") {{ item.name }}
</template>
...
export default {
  ...
  data () {
    return {
      todoArray: [
        {
          content: '完成vue-iview基本开发'
        },
        {
          content: '对vue-iview进行性能优化'
        },
        {
          content: '对vue-iview的细节进行优化'
        },
        {
          content: '完成vue-iview开发'
        },
        {
          content: '解决发现的bug'
        },
